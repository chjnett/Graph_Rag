...
- **2.1-a** (M) DeBERTa-v3 span-based 헤드 vs 1B급 seq2seq 두 후보 최소 구현체 프로토타입 (더미 데이터로 forward pass만 확인, 둘 다 3090 24GB에 여유 있게 적재됨)
  - **(신규) 모델 선택에 따른 파괴적 망각(catastrophic forgetting) 리스크 조건부 적용**: 학생 모델의 유일한 역할은 triple 추출(구조화 출력)이며, 다운스트림 QA의 최종 답변 생성은 학생 모델이 아니라 로컬 교사(Qwen2.5-32B)를 재활용한다(`graphrag_03_graph_construction.md` Phase 3.35-d). 따라서 **DeBERTa-v3 span-based 헤드를 선택하면** 애초에 자유 생성 능력이 없는 구조라 "일반 언어 능력 상실"이라는 리스크 자체가 성립하지 않는다. **1B급 seq2seq 생성형을 선택하면** 합성 triple 데이터로만 SFT할 경우 출력이 구조화 포맷에 과적합되어 다양한 도메인 텍스트에 대한 추출 강건성이 떨어질 위험이 있으므로(엄밀히는 "대화 능력 상실"이 아니라 "추출 강건성 저하"), 2.3의 보조데이터 비율 실험이 이를 완화하는 장치로 기능함을 명시
...
> 이 실험의 목적은 "일반 대화 능력 보존"이 아니라 — SciERC/WebNLG/FewRel은 모두 IE 포맷 데이터라 — 합성 triple 데이터 하나에만 SFT했을 때 생기는 **추출 포맷·도메인 과적합**(2.1-a 참고)을 완화하는 것. 결과는 Phase 2.5(FewRel 일반화 테스트)로 검증
