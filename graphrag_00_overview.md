...
> **⚠️ 리소스 제약 (2026-07 기준 확정)**: 실행 환경은 **RTX 3090 1장(24GB VRAM, 시간 무제한)**이고, **유료 API 예산은 사실상 $0**(대학생 개인 프로젝트, sanity-check용 소액만 선택적 허용). "교사 LLM"은 이 프로젝트 문서 전체에서 별도 언급이 없는 한 **로컬 오픈소스 강모델(RTX 3090 4bit 양자화 구동, 예: Qwen2.5-32B-Instruct-AWQ)**을 뜻하며, vLLM으로 서빙해 OpenAI 호환 API처럼 호출한다. baseline 인덱싱도 동일 로컬 모델로 재현한다.
...
- **로컬 오픈소스 교사(32B급)는 GPT-4o/Claude 등 최상위 proprietary 모델보다 약할 수 있어, 합성 데이터 품질 상한 자체가 원래 설계보다 낮을 가능성** — 예산 제약에 따른 의도적 트레이드오프임을 논문에 명시. 이는 모든 서브프로젝트에 영향을 미치는 근본적 제약
- 합성 데이터의 교사 모델 편향이 학생 모델에 전이될 위험, 인간 검수 골드셋의 단일 annotator 한계 — 상세: `graphrag_01_data_pipeline.md` §한계
