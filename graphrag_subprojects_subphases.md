...
>
> **2차 개정 (methodology 리뷰 반영)**: (1) 순환논증 방지 — Phase 1.6-d에 인간 검수 골드셋(30~50 triple) 라벨링 신설, Phase 3.7을 "교사-학생 일치율"과 "골드셋 실제 정확도" 두 지표로 분리. (2) Baseline 삼각비교 — Phase 3.6-c/4.5-c에 원 논문(GPT-4 기반) 보고 수치를 앵커로 병기하는 단계 추가. (3) 표본 크기 — 고정 평가 서브셋을 50~100 → 150~200청크로 확대, 도메인별 분석은 탐색적(exploratory)으로 한정.
...
- **1.2-f** (S) 도메인별 균등 샘플 **150~200개**(도메인당 약 8~11개, 리뷰 반영으로 50~100개에서 확대)를 **고정 평가 서브셋**으로 별도 태깅·저장 (Phase 3.7에서 재사용)
...
- **1.6-d** (M, 신설/순환논증 방지) 고정 평가 서브셋(1.2-f) 중 30~50 triple을 무작위 층화 추출해 저자 본인이 원문 `source_span` 대조로 직접 gold 라벨링(정답 엔티티/관계 여부 수기 판정). 단일 annotator 한계는 datasheet에 명시. 이 골드셋이 있어야 Phase 3.7에서 "교사-학생 일치율"과 분리된 "실제 정확도"를 산출할 수 있음(교사=학습신호이자 평가정답이 되는 순환논증 방지)
- Done when: 최종 JSON 데이터셋 파일 + datasheet 문서 + 인간 검수 골드셋(30~50 triple, 라벨 포함) 존재, 서브프로젝트 2에 바로 투입 가능
...
- **3.6-c** (S, 신설/삼각비교) baseline 원 논문(Microsoft GraphRAG/LightRAG 등)이 GPT-4 계열로 보고한 QA/커버리지 수치(가능한 범위)를 별도 참고 열로 정리 — 우리 baseline 재현(로컬 LLM)과 원 논문 수치 사이 괴리 판단용 삼각비교 자료, Phase 4.5에서 재사용
- Done when: 비교표 + 원 논문 앵커 열 + 구조적 유사성에 대한 짧은 해석 문단 (※ 이는 recall이 아니라 구조 비교임을 리포트에 명확히 표기)

### Phase 3.7 — 교사-학생 일치율 & 골드셋 실제 정확도 계산 (신설, 2차 개정으로 지표 분리)
- **3.7-a** (S) Phase 1.2-f 고정 평가 서브셋(150~200청크, 교사 모델 참조 triple)과 Phase 3.1의 학생 모델 triple 중 동일 서브셋을 매칭
- **3.7-b** (M) Phase 1.4와 동일한 매칭 규칙으로 엔티티 recall, 관계 recall 계산 → **교사-학생 일치율**로 명명 (교사 출력을 정답으로 쓰는 지표이므로 "실제 정확도"가 아님을 리포트에 명시)
- **3.7-c** (S) 도메인별 일치율 분포 확인, 단 도메인당 표본 10개 내외이므로 **탐색적(exploratory) 해석**으로 한정한다고 리포트에 명기
- **3.7-d** (M, 신설) Phase 1.6-d 인간 검수 골드셋(30~50 triple)과 Phase 3.1 학생 모델 triple을 동일 매칭 규칙(1.4 기준)으로 비교 → **골드셋 실제 정확도(precision/recall)** 산출
- **3.7-e** (S, 신설) 3.7-b(교사-학생 일치율)와 3.7-d(골드셋 실제 정확도)를 나란히 비교 — 괴리가 크면 교사 모델 편향이 학생에게 전이된 것으로 해석, Limitations/Discussion에 반영할 근거로 기록
- Done when: 교사-학생 일치율 리포트 + 골드셋 실제 정확도 리포트 + 두 지표 비교 분석 완성 — 설계서 §6.3 "품질" 축 핵심 수치 확정, 순환논증 없이 실제 품질 주장 가능
...
- **4.5-c** (S, 신설/삼각비교) Phase 3.6-c에서 정리한 baseline 원 논문 보고 수치를 참고 앵커 열로 최종 Table에 병기
